...
def train(epo_num=20, show_vgg_params=False):
    # vis = visdom.Visdom()
    ...
    optimizer = optim.SGD(fcn_model.parameters(), lr=1e-6, momentum=0.7)
    all_train_iter_loss = []
    all_test_iter_loss = []
    ...
    for epo in range(epo_num):
        ...
        if np.mod(epo, 1) == 0:
            torch.save(fcn_model, 'checkpoints4/fcn_model_{}.pt'.format(epo))
            print('saveing checkpoints4/fcn_model_{}.pt'.format(epo))
