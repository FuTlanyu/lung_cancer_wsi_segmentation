...
class WSI(object):
    ...
    def read_tumor_wsi(self, wsi_path, mask_path):
        ...
        try:
            ...
            #     for j in range(5568):
            #         if (self.mask[i][j] != [0, 0, 0, 255]).all() :
            #             print("(%d, %d)" % (i, j))
    ...
    def find_roi_tumor(self, flag, tumor_patches):
        ...
        cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_image_open.png', np.array(image_open))
        ...
        # for coor_x, coor_y in zip(x_list, y_list):
        #     coor_x_8 = coor_x*(2**4)
        #     coor_y_8 = coor_y*(2**4)
        #     patch = self.wsi_image.read_region((coor_x_8, coor_y_8), 0, (util.PATCH_SIZE, util.PATCH_SIZE))
        #     mask_pt = self.gt_mask.read_region((coor_x_8, coor_y_8), 0, (util.PATCH_SIZE, util.PATCH_SIZE))
        #     # print(mask[0].shape)
        #     mask_ts = mask[coor_y:(coor_y+32), coor_x:(coor_x+32)]
        #     # print(mask_ts.shape)
        #     # print(mask_ts)
        ...
        #     print(white_pixel_cnt_ts)
        #
        #     if white_pixel_cnt_ts > (1024 * 0.30):
        #         r, g, b, a = cv2.split(patch_array)
        #         merged = cv2.merge([r, g, b])
        #         # print(merged)
        #         misc.imsave(util.THESIS_PATCH_DIR + 'patch_' + str(flag) + '.png', merged)
        #         misc.imsave(util.THESIS_MASK_DIR + 'mask_' + str(flag) + '.png', mask_array)
        #         flag = flag + 1

        # res = np.array(res)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_bit_mask', res)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_mask.png', self.mask)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_image_close.png', np.array(image_close))
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_image_open.png', np.array(image_open))

        # rgb_bbox = self.rgb_image.copy()
        # for i, bounding_box in enumerate(bounding_boxes, mask):
        ...
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_contour.png', contour_rgb)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_contour_mask.png', contour_mask)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_bbox.png', rgb_bbox)
        # cv2.imwrite(os.path.join(util.THESIS_FIGURE_DIR, wsi_name) + '_hsv.png', hsv)
        ...
        for coord in coor_list:
            ...
            if self.wsi_type == 'TRAIN':
                ...
                if white_pixel_cnt_ts > (level4_psize * level4_psize * tissue_area):
                    ...
                    #         break
                    #     for j in range(512):
                    #         if (self.mask[i][j] != [0, 0, 0, 255]).any() :
                    #             has_white_areas = has_white_areas + 1
                    #             break_flag = True
                    #             break
                    ...
                    # patch_array size: (512, 512, 3)  mask_array size: (512, 512)
                    imageio.imwrite(util.train_img + str(flag) + ".png", patch_array)
                    imageio.imwrite(util.train_mask + str(flag) + ".png", mask_array)

                    # imageio.imwrite('E:/python/program/Dachuang/Unet-like-model/UnetSeg-Pytorch/data/demo/imgs/' + str(flag) + ".png", patch_array)
                    # imageio.imwrite('E:/python/program/Dachuang/Unet-like-model/UnetSeg-Pytorch/data/demo/masks/' + str(flag) + ".png", mask_array)
                    if self.vis:
                ...
                white_pixel_cnt_ts = cv2.countNonZero(mask_ts)
                if white_pixel_cnt_ts > (level4_psize * level4_psize * 0.02):
    ...
    def get_patch_and_mask(self, bounding_boxes, tissue_mask, flag, tumor_patches):
        ...
        for bounding_box in bounding_boxes:
            ...
            # img = self.wsi_image.read_region((x * 2**4, y * 2**4), 4, (bounding_box[2], bounding_box[3])).convert('RGB')
            # img = np.array(img)
            # imageio.imwrite('E:/UnetSegPy-Data/data/demo/imgs/' + f'{str(x)}_{str(y)}.png', img)
        ...
        for bounding_box in new_bboxes:
            ...
            if util.LEVEL_USED == 0:
                ...
                stride = int(256 / 2 ** (4 - util.LEVEL_USED))
        ...
        #     print(f'Right-Down: ({xr}, {yd})')
        #     # 添加一个最大的bounding box
        #     self.bpatches.append((xl, yu, xr-xl, yd-yu))
        #     img_max = self.wsi_image.read_region((xl * 2**4, yu * 2**4), 4, (xr-xl, yd-yu)).convert('RGB')
        #     img_max = np.array(img_max)
        #     imageio.imwrite('E:/UnetSegPy-Data/data/demo/imgs/' + f'{str(self.index)}max_{str(x)}_{str(y)}.png', img_max)
        #
        ...
        #     y_range = (yd-yu) * 2**4
        #
        #     row = math.ceil(y_range / util.PATCH_SIZE)
        #     col = math.ceil(x_range / util.PATCH_SIZE)
        #     img_num = row * col
        #     print('Extract %d patches from image %s' % (img_num, str(self.index)))
        #     self.f.write('Extract %d patches from image %s\n' % (img_num, str(self.index)))
        #
        #     for index in range(int(img_num)):
        #         x = int(x_coor + index % col * util.PATCH_SIZE)
        #         y = int(y_coor + int(index // col) * util.PATCH_SIZE)
        #
        #         img = self.wsi_image.read_region((x, y), 0, (util.PATCH_SIZE, util.PATCH_SIZE)).convert('RGB')
        #         gt_mask = self.mask_image.read_region((x, y), 0, (util.PATCH_SIZE, util.PATCH_SIZE)).convert('RGB')
        #
        #         img = np.array(img)
        ...
        #
        #         print('test patch index: ', index)
        #
        #         self.bpatches.append((int(x/2**4), int(y/2**4), util.PATCH_SIZE/2**4, util.PATCH_SIZE/2**4))

            # 随机采点
            # =============================================================================
            # num_x = int(bounding_box[2] * 0.15)     # 0.1用于控制点的密集程度 0.3
            # num_y = int(bounding_box[3] * 0.15)
            #
            # while flag_x != num_x:
            #     random_x = random.randint(x, end_x)
            #     # set 创建一个无序不重复集合
            #     x_list_ts = set(x_list)
            #     if random_x not in x_list_ts:
            #         x_list.append(random_x)
            #         flag_x += 1
            #
            # while flag_y != num_y:
            #     random_y = random.randint(y, end_y)
            #     y_list_ts = set(y_list)
            #     if random_y not in y_list_ts:
            #         y_list.append(random_y)
            #         flag_y += 1
            # =======================================================================================
            # flag is the next patch number

            # flag, tumor_patches = self.process_level(x_list, y_list, util.LEVEL_USED, tissue_mask, flag, tumor_patches)
            # if flag > self.patch_index:
            #     break;
...
# def get_tumor_image():
#     wsi = WSI()
#     wsi_path = 'D:/CAMELYON-19/Data/TrainingData/Train_Tumor/99.tif'
#     mask_path = 'D:/CAMELYON-19/Data/TrainingData/Ground_Truth/Mask/99_mask.tif'
#     if wsi.read_tumor_wsi(wsi_path, mask_path):
